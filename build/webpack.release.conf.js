...
const webpackConfig = merge(baseWebpackConfig, {
  module: {
    rules: utils.styleLoaders({
      sourceMap: config[configEnv].productionSourceMap,
      extract: true,
      usePostCSS: true
    })
  },
  devtool: config[configEnv].productionSourceMap ? config[configEnv].devtool : false,
  output: {
  ...
  plugins: [
    ...
    new webpack.DefinePlugin({
      'process.env': env,
      'process.config': require('../src/config/release')
    }),
    new UglifyJsPlugin({
      uglifyOptions: {
        compress: {
    ...
    new OptimizeCSSPlugin({
      cssProcessorOptions: config[configEnv].productionSourceMap
    ...
    new HtmlWebpackPlugin({
      staticPath: 'http://m.static.stg.laiyifen.com',
    ...
    new webpack.optimize.CommonsChunkPlugin({
      name: 'vendor',
      filename: 'my-static/js/vendor.js?v=' + new Date().getTime(),
      minChunks (module) {
